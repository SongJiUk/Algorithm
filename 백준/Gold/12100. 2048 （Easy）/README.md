# 어우 어렵네 어려워.. 처음에 문제를 잘못 이해했다.
말 그대로 처음에 문제를 잘못 이해했다. 위로 올리면 모든수가 한번에 위로 가야되는데 하나씩 옮기고있었다... </br>
문제를 다시 파악한 후에 값이 충분히 작아서 DFS로 돌려도 상관없을거같아서 DFS로 돌렸다. </br>
맨 처음에 큐에 넣어서 할까 고민했는데. 도저히 생각이 안나서 그냥 for문돌렸다가 실패하고 다시 큐로 돌아왔다. </br>
내가 한방법은 이렇다. 재귀를 돌려가면서 상, 하, 좌, 우  순서대로 돌렸다. </br>
예를 들어 위로 가야되는거면. 위에서부터 큐에 넣어준다음. 큐를 비워주면된다. </br>
만약 MAPS의 좌표값이 0이면 큐를 넣어주면되고, 좌표값이 큐 값과 같으면 *2를 해주면되고, 0도아니고 같지도 않으면 그 다음값에 큐의값을 넣어주면 된다.</br>
너무너무너무 깊게 생각해서그런가 시간이 좀 걸렸는데. 이제부터 골드문제만 풀다보면 계속 오래걸릴것 같긴하네... </br>
