...
일단 나는 1번부터 N번을 X로 보내고, X에서 해당번호로 돌아오는값들을 더해서 최대값을 구해줬다. </br>
처음에 메모리초과가 나길래 아 이건 방문처리를 안해줘서 이런거구나 하고 방문처리를 해봤는데 bool값으로하면 방문이 안되는곳이있었다. </br>
그래서 int형으로 선언해주고 해당 값이 최소값이 아니라면 방문할 필요없이 만들어버렸다. </br>
생각보다 빠르게 풀었는데, 방문처리를 고민하느라 조금 더 걸린것같다. 여러군데에서 한곳을 방문해야하는데 최소값, 최대값을 찾아야하는경우에는 int형으로 해줘야겠다. </br>
