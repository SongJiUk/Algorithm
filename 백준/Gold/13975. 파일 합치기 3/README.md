# 왜 자료형을..
항상 정수가 나오면 int형으로 초기화하는 버릇을 좀 고쳐야될것 같다.</br>
아이패드에 long long으로 해야된다고 적어놓고, 자동적으로 int형으로 초기화해서 틀려버렸네.. </br>
한방에 맞출 수 있는걸 왜 자꾸 자료형을 잘못 선언해서 틀릴까.. </br>
이 문제는 우선순위큐로 풀었다. </br>
내림차순으로 정렬되게 만든 후, 두개의 값을 뽑아 더해가고 그 값을 다시 넣어주고(다른 곳에 이 값을 저장), 큐가 빌떄까지 돌려준 후에.</br>
저장한 값을 모두 더해주면 최솟값이 된다. </br>
내림차순으로 정렬하는 이유는 작은 수끼리 더해야 작은수가 나오기 떄문.</br>
