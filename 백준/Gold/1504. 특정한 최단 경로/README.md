# 다익스트라 업그레이드 버전
고민하는시간보다 구현하는시간이 더 걸렸다. </br>
방법은 몇가지 없다. 이 방법을 구해주면 된다. </br>
만약 2,3번을 지나서 4번으로 가야된다고하면 </br>
1 - 2 - 3- 4 or 1- 3 -2 - 4 이다. </br>
그래서 다익스트라로 계산들을해서 값을 저장해준뒤 최솟값을 찾아 출력하면된다. </br>
여기서 주의할점은 최대값을 너무 크게하면 합이 21억이 넘을수도있어서 정답이 아닐수도 있다.</br>
dist[_num] = 0; 이거 잊지말고. </br>
