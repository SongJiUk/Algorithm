# BFS로 풀었는데 DP네..
조금 당황스럽네.. 뭐 어떻게 풀든 상관은 없지만 DP가 확실히 빠를것 같긴하다. </br>
점화식을 보니 쉽게 이해가 가긴하네.. </br>
DP[i][j][0] = DP[i][j-1][0] + DP[i][j-1][1]; </br>
DP[i][j][1] = DP[i-1][j-1][0] + DP[i-1][j-1][1] + DP[i-1][j-1][2];</br>
DP[i][j][2] = DP[i-1][j][1] + DP[i-1][j][2]; </br>
이 문제는 즐겨찾기 해놓고 까먹을때쯤에 다시 DP로 풀어봐야겠다. </br>
