# BFS, DFS 다 필요
일단 이 문제를 푸려면 조합이 필요했다. </br>
쉽게 말해서 총 N개(N<=10) 이니까 N개의 모든 경우의수가 필요하다. </br>
예를 들어서 1~6까지라고 치면 {1},{2},{3},{4},{5},{6} ~ {1,2,3,4,5,6} 이런식으로. </br>
근데 문제의 조건에 무조건 두개의 구역으로 나누어져야한다고했으니까. {1}, {2,3,4,5,6} ~ {1,2,3,4,5} ,{6} 이렇게 나눠줘야한다. </br>
나는 처음에 받을때 2차원배열 bool값으로 두 구역이 연결되어있는지 체크해줬다. </br>
그리고 1부터 조합을 찾기위한 재귀함수를 돌려줬다. 돌려주고. A,B로 나누어진다고 하면, 그 A끼리 연결되어있는지, B끼리 연결되어있는지를 확인해줬다. </br>
확인해줄때는 BFS로 찾아주었고, 확인한 후 맞다면 두 조합의 차이를 계산해주고, 최솟값을 저장해줬다. </br>

난이도가 올라가다보니까 하루에 두세문제를 푸려고 마음먹어도 생각보다 쉽지않다. </br>
그래도 하나씩 풀리는거보면 재밌고 뿌듯하네. 
