# 바보같은 짓을 했다..
배열로 만들고 상하좌우 검사해주면서 인접한 수를 지워가는데 첫번째 열이랑 마지막열이랑은 계산해주면 안되는데 바보같이 계산해서 헤맸다.. </br>
처음부터 원 그려보고 할걸.. </br>
어쨋든 이런 참신한 문제도 있구나 라는 생각도 든다. </br>
아! 그리고 +1, -1을 하는 부분에서 숫자가 같으면 아무것도 하지 않는것도 헤맸다..</br>
