# 다익스트라도 방문체크가 필요하긴하구나.
나는 다익스트라 알고리즘은 우선순위큐로 최단경로를 찾는거라 방문한 노드는 재방문 안할줄알았는데. </br>
방문체크가없으면 계산한 노드가 다시 연산을할줄 몰랐다. </br>
그냥 어떻게든 방문체크는 해주면 좋을것같다. </br>
