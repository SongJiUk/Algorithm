# BFS !
처음엔 안에있는 0들을 바꾸고싶지않아서 밖에있는 모든값들을 -1로 바꾸고 시작했는데. 쉽게 되지가않았다. </br>
그래서 그냥 0,0부터 시작해서 0을 만나면 큐에 넣어주고 1을 만나면 0으로 바꾸고 큐에 넣지 않는식으로 반복문을 돌렸다. </br>
생각보다 쉬운 문젠데 딴짓많이하다 시간을 많이 잡아먹혔다 ..</br>
