# DP너무 어려워...
DP문제 진짜 너무 어렵다... </br>
그림으로 그려보고 찾았는데도 풀다가 도저히 모르겠어서 힌트를 얻어서 풀었다. </br>
DP[한개짜리 알약의 개수][반개짜리 알약의 개수] 이런식으로 해서 풀었다. </br>
DP[i][j] = DP[i-1][j+1] + DP[i][j-1] 이런식으로 </br>
DP[3][0] 이라고하면 재귀를 돌려서 i와,j가 0이될때까지 재귀를 돌려주고, 계산해주면 된다. </br>
