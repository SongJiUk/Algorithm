# 삽입 삭제가 빠른 자료구조는 Linked list!!
하도 안쓰다가 쓰니까 헷갈려서 어떻게 쓰는지 검색해봤다 ㅋㅋㅋㅋ </br>
맨 마지막 노드를 쓰기 위해서는 end()로 받아주고 하나 빼줘야된다. </br>
그리고 erase는 삭제한 뒤의 위치를 반환해주기 때문에 그것 또한 신경 써줘야한다.</br>
그리고 이제부터는 실버2 ~ 골드 문제를 위주로 풀어야될 것 같다. 이제 경험치가 많이 안오르네.. 화이팅 해보자
