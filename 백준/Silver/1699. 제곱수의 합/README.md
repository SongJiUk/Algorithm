# DP!
이건 그림으로 그려보니까 누가봐도 DP다 </br>
계속 비교를 해줘야되는데 일단 모든 수는 1^2으로 만들 수 있다. 그렇게 초기화를 해주고. </br>
min을 사용하여 제곱수부분에서만 체크해주면 된다. </br>

DP[i] = min(DP[i], DP[i- 제곱수^2] + 1); </br>
이렇게 해주면 된다. 
