# 생각보다 고전했다.
난이도가 조금씩 올라갈수록 생각보다 쉽지 않다. </br>
그래도 DP에 대한 감은 조금씩 잡히는것 같다. </br>
일단 이건 이렇게 풀었다. </br>
1. 00으로 시작하는 조건 </br>
2. 1로 시작하는 조건 </br>

여기서 DP[N] 이라고 하면 1번의 조건은 DP[N-2] 이다. 그리고 2번의 조건은 DP[N-1]이 된다. </br>
두가지 조건밖에 없으니 DP[N] = DP[N-2] + DP[N-1] 을 하면 조건이 성립이된다. </br>
