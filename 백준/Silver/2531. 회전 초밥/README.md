# 슬라이딩 기법..
처음에 메모리 초과가 뜨길래 vector와 set을 너무 남발했나 싶어서 윈도우 슬라이딩 기법으로 해봤다.</br>
set을 매개변수에 주소값을 올려 보내봤는데 이번엔 시간초과가 어떻게 해야되나 고민하다가 도저히 모르겠어서 힌트를 몇개 봤다.</br>
내가 했던거에다가 set을 넣어주는방식이 아닌, 그냥 bool 배열을 하나 선언을해서, 중복되는 값이 있으면 전체값에서 빼주고, 쿠폰값이 있으면 쿠폰값을 하나 빼주는 방식으로 해주면 됐다. </br>
다 왔는데 아쉽긴하다.. 그래도 다음에 이런 문제가 나오면 bool배열 선언해서 해야겠다. </br>
