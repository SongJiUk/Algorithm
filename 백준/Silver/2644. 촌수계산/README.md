# 나는 BFS로 했다.
처음 주어지는 두개의 값중 아무거나 선택해서, 큐에 count와 함께 넣어주고 for문을 돌려서 해당 값이 자식이되거나, 부모가되는 경우를 찾아 큐를 돌려줬다.</br>
그리고 두개의 값중 선택하지 않은 값과 같다면 break되게하고 count를 출력해주고, while문이 그대로 끝나버린다면 -1를 출력해줬다. </br>
DFS로도 풀어보고싶은데, 다음에 한번 해봐야겠다. 뭐 BFS로 성공하면 더 좋은거니까. </br>
BFS는 해당지역을 방문했는지 않헸는지 구분해주는게 제일 중요한것 같다.</br>
아예 큐에 넣지 않도록 해버리면 반복문의 개수가 달라져버리니 이제부턴 아예 넣지 않는방법으로 해야겠다. </br>
