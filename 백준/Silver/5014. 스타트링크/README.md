# BFS로 쉽게쉽게 풀면 된다.
방문처리를 해주면서 올라가는 경우와 내려가는경우를 비교를 해주면서 풀어가면된다. </br>
생각보다 쉽게 풀려서 당황했는데, 동적할당으로 문제를 푸니까 배열의 수를 넘어가는 경우도없어서 좋네..</br>
골드만 푸려했는데 실버 잘못눌러서 풀었는데, 골드랑 실버랑 난이도차이가 극심한것같다.</br>
암튼 취업까지 힘내보자. 
